...
const BookDetailsPage = () => {
    ...
    const filteredRecommendations = bookCollection.filter(
        (recommendedBook) =>
            recommendedBook.genre === book.genre &&
        ...
        <div className="book-details-container">
            ...
            <div className="recommendations-container">
                ...
                <ul className="recommendations-list">
                    {filteredRecommendations.slice(0, 4).map((book) => (
                        <Book
                            key={book.title}
                            {...book} // Pass all book data
                            // Additional props for wishlist and cart functionality
                            addToWishlist={addToWishlist}
                            removeFromWishlist={removeFromWishlist}
                            isInWishlisted={wishlist.some(
                                (b) => b.title === book.title
                            )}
                            addToCart={addToCart}
                            removeFromCart={removeFromCart}
                            isInCart={cart.some((b) => b.title === book.title)}
                        />
                    ))}
                </ul>
            </div>
